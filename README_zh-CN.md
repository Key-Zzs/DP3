...
- 默认相机：`head`。
- 深度来源：显式 `--depth-source native_depth|ffs_stereo`，默认是保持旧命令
  行为的 `native_depth`。`native_depth` 使用录制的 RealSense `uint16` depth；
  `ffs_stereo` 使用录制的左右 IR 和现有 PointCloudBuilder FFS backend。
...
`--depth-source native_depth` 保持 `--builder-config` 可选，不请求 IR sidecar
字段，也不会初始化 TensorRT 或 FFS vendor runtime。`--depth-source ffs_stereo`
必须提供显式 FFS Builder YAML。两个 canonical Flexiv Builder config 为：
...
最终 resolved config。CLI 的 backend、artifact id、precision、optimization
level 和 workspace 必须与 YAML 一致；冲突会直接 fail-fast。
...
两个阶段调试脚本都支持与 exporter 相同的显式 depth-source contract。调试 FFS
...
如果输入是 FFS 派生 zarr，`debug_zarr_pointcloud_stages.py` 会从 `.zattrs` 自动
读取 `depth_source`、backend、artifact contract 和 Builder config，因此会复现
FFS 路线，不会回退到 native depth。也可以传入 `--depth-source` 和 `--ffs-*` 参数
覆盖并交叉校验 zarr 中保存的值。
...
perception-only 入口也支持显式 FFS 路线。传入包含实时相机内参和 artifact 路径的
完整 FFS Builder YAML，并指定 `--depth-source ffs_stereo`；程序启动时会打开左右
IR 流，并在打开相机前完成 manifest/artifact 预检。`--ffs-*` 参数必须与 YAML 一致。
例如：
